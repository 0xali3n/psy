...
│  │                                                                                     │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐         │   │
│  │  │   Frontend   │  │  Identity    │  │  Encryption  │  │  Local       │         │   │
│  │  │   (HTML/JS)  │  │  Manager     │  │  Engine      │  │  Storage     │         │   │
│  │  │              │  │  (ED25519)   │  │(XChaCha20)   │  │(localStorage)│         │   │
│  │  │  - UI/UX     │  │  - Keypair   │  │  - Encrypt   │  │  - Keys      │         │   │
│  │  │  - QR Code   │  │  - Hash      │  │  - Decrypt   │  │  - Identity  │         │   │
│  │  │  - Chat UI   │  │  - Sign      │  │  - Nonce     │  │  - State     │         │   │
│  │  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └──────────────┘         │   │
│  │         │                  │                  │                                   │   │
│  │         └──────────────────┴──────────────────┘                                   │   │
│  │                              │                                                    │   │
...
│                                 ▼                                                        │
│  ┌─────────────────────────────────────────────────────────────────────────────────────┐   │
│  │                           API SERVER LAYER (Rust/Actix-web)                        │   │
│  ├─────────────────────────────────────────────────────────────────────────────────────┤   │
│  │                                                                                     │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐         │   │
│  │  │   HTTP       │  │  Message     │  │  Identity    │  │  ZK Proof    │         │   │
│  │  │   Router     │  │  Store       │  │  State       │  │  Engine      │         │   │
│  │  │              │  │              │  │              │  │              │         │   │
│  │  │  - /send     │  │  - Messages  │  │  - Public    │  │  - CFC Proof │         │   │
│  │  │  - /read     │  │  - Threads   │  │    Keys      │  │  - Verify    │         │   │
│  │  │  - /identity │  │  - CSTATE    │  │  - Hashes    │  │  - EndCap    │         │   │
│  │  │  - /cstate   │  │  - Keys      │  │  - Nonces    │  │  - SimProver │         │   │
│  │  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘         │   │
│  │         │                  │                  │                  │                 │   │
│  │         └──────────────────┴──────────────────┴──────────────────┘                 │   │
│  │                              │                                                    │   │
...
│  │                                                                                     │   │
│  │  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐               │   │
│  │  │   Encryption     │  │   Commitments    │  │   State          │               │   │
│  │  │   Service        │  │   Service        │  │   Management     │               │   │
│  │  │                  │  │                  │  │                  │               │   │
│  │  │  XChaCha20-      │  │  Poseidon2-style │  │  Merkle Tree     │               │   │
│  │  │  Poly1305        │  │  Hashing         │  │  Construction    │               │   │
│  │  │                  │  │                  │  │                  │               │   │
│  │  │  - 256-bit key   │  │  - Message       │  │  - CSTATE Root   │               │   │
│  │  │  - 192-bit nonce │  │    Commitment    │  │  - Thread Roots  │               │   │
│  │  │  - AEAD          │  │  - Plaintext     │  │  - State Updates │               │   │
│  │  │  - Auth Tag      │  │    Hash          │  │  - Rollback      │               │   │
│  │  └──────────────────┘  └──────────────────┘  └──────────────────┘               │   │
│  │                                                                                     │   │
│  └─────────────────────────────────────────────────────────────────────────────────────┘   │
│                                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────────────────┐   │
│  │                    FUTURE: Psy Protocol Integration Layer                           │   │
│  ├─────────────────────────────────────────────────────────────────────────────────────┤   │
│  │  Realm → EndCap Submission  │  UCON → CSTATE Storage  │  DA Miner → Blob Storage  │   │
│  │  CLEAF → CFC Whitelist      │  Indexer → Message Query                            │   │
│  └─────────────────────────────────────────────────────────────────────────────────────┘   │
│                                                                                             │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
```
Client Side:
  1. Encrypt message (XChaCha20-Poly1305)
  2. Compute commitment (Poseidon2-style)
  3. Send to server

Server Side:
  1. Get current CSTATE root
  2. Update CSTATE (Merkle tree)
  3. Generate ZK proof (CFC proof, simulated)
  4. Create EndCap (proof + signature + nonce)
  5. Store message & update state
...
**Flow Description:** Messages are encrypted client-side before transmission. The server never sees plaintext. Each message generates a commitment hash and a ZK proof that verifies the state transition is valid. The CSTATE (Contract State) is a Merkle tree that tracks all message commitments, creating an immutable, verifiable history. Recipients decrypt messages locally and verify both the commitment and ZK proof to ensure message integrity and state consistency.
...
│  Commitment  │  │  Commitment │  │Commit. │  │  Commitment │
│  "0xabc..."  │  │  "0xdef..." │  │"0x123" │  │  "0x456..." │
└──────────────┘  └─────────────┘  └────────┘  └─────────────┘

Each message adds a new commitment to the thread's Merkle tree.
The thread root updates, which updates the CSTATE root.
This creates an immutable, verifiable history of all messages.
```
